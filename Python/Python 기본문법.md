...
모듈이란 함수나 변수 또는 클래스를 모아 놓은 파일으로 모듈은 다른 파이썬 프로그램에서 불러와 사용할 수 있게끔 만든 파이썬 파일이라고도 할 수 있다

모듈을 사용하기 위해서는 import를 해줘야 하는데 
...
위와 같이 여러 방법으로 모듈을 import 할 수 있다

각 방법 별 사용방법 차이를 보면

1. import 모듈명 : 모듈명.함수
1. from 모듈명 import 함수 : 함수
1. from 모듈명 import * : 함수
1. from 모듈명 import 함수 as 별명 : 별명.함수

이렇게 구분할 수 있다

# 패키지

패키지란 특정 기능과 관련이 있는 여러 모듈을 모아놓은 폴더를 뜻한다

패키지를 사용하기 위해 import 하는 방법은

모듈을 import하는 방식과 비슷하며 .으로 패키지 폴더 안에 있는 폴더나 파일들에 경로를 지정해 특정 모듈들을 선택하는게 가능하다
다만 *으로 모두를 import 시키는 건 중복되는 함수나 모듈이 있을 수 있으므로 주의해야 한다

## init

__init __.py 파일은 해당 디렉터리가 패키지의 일부임을 알려주는 역할을 하는 파일로 특정 디렉터리의 모듈을 *를 사용하여 import할 때에는 
~~~
__all__ = ['ex']
~~~
위 코드와 같이 디렉터리의 __init __.py 파일에 __all __ 변수를 설정하고 import할 수 있는 모듈을 정의해 주어야 한다
